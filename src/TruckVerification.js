...
function TruckVerification() {
  ...
  useEffect(() => {
    if (feildcrn) {
      ...
      axios.get(`http://3.109.145.125:9001/truckverify?feildcrn=${feildcrn}`)
        .then((response) => {
  ...
  const sendAcceptanceEmail = (truckNumber,name,email) => {
    // Replace 'your_email_service_id' and 'your_template_id' with your EmailJS service and template IDs
    emailjs.send(
      'service_qucoi14',
      'template_vcrqn4p',
      {
        trucknumber:truckNumber,name:name.toUpperCase(),email:email
      },
      'q5nhqMCBFfD5LBjv9' // Replace with your EmailJS user ID
    )
      .then((response) => {
        console.log('Email sent for truck acceptance:', response);
      })
      .catch((error) => {
        console.error('Error sending email for truck acceptance:', error);
      });
  };
  
  const sendRejectionEmail = (truckNumber,name,email) => {
    // Replace 'your_email_service_id' and 'your_template_id' with your EmailJS service and template IDs
    emailjs.send(
      'service_n5ywh2g',
      'template_bib0ylu',
      ...
      'aTym9jehOT7GsFLMt' // Replace with your EmailJS user ID
    )
      .then((response) => {
        console.log('Email sent for truck rejection:', response);
      })
      .catch((error) => {
        console.error('Error sending email for truck rejection:', error);
      });
  };
  const handleVerify = (truckId,truckNumber,name,email) => {
    ...
    axios.put(`http://3.109.145.125:9001/truckUpdate/${truckId}`, { status: 'Completed', verificationDate: verificationDate })
      .then((response) => {
        ...
        sendAcceptanceEmail(truckNumber,name,email);
      })
      .catch((error) => {
  ...
  const handleReject = (truckId, truckNumber,name,email) => {
    // Send a request to update the status to "Rejected" for the given truck
    axios.put(`http://3.109.145.125:9001/truckUpdate/${truckId}`, { status: 'Rejected' })
      .then((response) => {
        // Update the status in the frontend
        const updatedTruckData = truckData.map((truck) => {
          if (truck.id === truckId) {
        ...
        setTruckData(updatedTruckData);
        sendRejectionEmail(truckNumber,name,email);
      })
      .catch((error) => {
        console.error('Error updating status:', error);
      });
    ...
    <video src={truck.truckVideoUrl} alt={`Truck ${truck.truckNumber} Odometer`} className="image-in-field"/><br></br> 
      ...
      <b>Truck Video</b><br></br> 
            ...
            <div className="buttons">
              <button className="btn btn-success ms-3"onClick={() => handleVerify(truck.id,truck.truckNumber,truck.name,truck.email)}>Verify Now</button>
              <button className="btn btn-danger ms-3"onClick={() => handleReject(truck.id,truck.truckNumber,truck.name,truck.email)}>Reject</button>
            </div>
          ) : null}
            </div>
